...
/// ```
///
/// When Xcode shall be used to compile or run the server code, an Xcode post-build script is needed to surface the generated API client code because it is generated outside the package folder. Add the following script as a post-build script to the main scheme of your package in Xcode and be sure to select the 'App' target under 'Provide build settings from':
/// ```
